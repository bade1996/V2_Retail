...
try:
    ...
    conf = SparkConf().setMaster("local[8]").setAppName("3Ingestion_Previous").set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")\
                .set('spark.local.dir', path+"/dump")\
                .set("spark.executor.memory","30g")
    sc = SparkContext(conf = conf)
    sqlctx = SQLContext(sc)
    spark = SparkSession.builder.appName("3Ingestion_Previous").getOrCreate()
    ...
    stage1_Rolling = sqlctx.read.csv(path+"/Config/stage1_Rolling.csv",header="true")  #consists name of tables whose FY will not be checked
...
#     stage1_Rolling.write.mode(owmode).save(hdfspath+"/Stage1/Stage1_Rolling")     #consists name of tables whose data will be refreshed everytime
    
    stage2.write.mode(owmode).save(hdfspath+"/Stage2/Stage2")
    stage2.write.mode(owmode).save(hdfspath+"/"+DBET+"/Stage2/Stage2")
    sqlctx.read.parquet(hdfspath+"/Stage1/Stage1_1").show()
    sqlctx.read.parquet(hdfspath+"/Stage1/Stage1_2").show()
    sqlctx.read.parquet(hdfspath+"/Stage2/Stage2").show(500)
    exit()
    ''
...
#             sys.exit()
    ...
    for j in range(0,df.count()):
        ...
        table = table.select(*(col(x).alias(re.sub('[)+]','', x)) for ix,x in enumerate(table.columns)))
        ############# FISCAL YEAR ADDED
        #table = table.withColumn("FiscalYear",newFY)
        table.cache()
        
        table.write.mode(owmode).save(hdfspath+"/"+DBET+"/Stage1/"+tbname1)
        
        prevTab = sqlctx.read.parquet(hdfspath+"/"+DBET+"/Stage1/"+tbwOyr+"20172018")  #Previous Table Read
        prevTab.cache()
