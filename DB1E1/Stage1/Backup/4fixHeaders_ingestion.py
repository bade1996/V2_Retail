...
try:
    ...
    conf = SparkConf().setMaster('local[8]').setAppName("4Fix_headers+FiscalYear").set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")\
...
#     print("OldFiscalYr"+oldFY)
    ...
    prevTab.write.partitionBy("yearmonth").mode(owmode).save(hdfspath+"/"+DBET+"/Stage1/"+tbwOyr+tbwOyr3+"1") ##
    ...
    prevTab.show()
    exit()
    prevTab=prevTab.withColumn("yearmonth",concat(prevTab.fiscalyear,month(prevTab.date_)))
    prevTab.write.partitionBy("yearmonth").mode(owmode).save(hdfspath+"/"+DBET+"/Stage1/"+tbwOyr) ##
    exit()
