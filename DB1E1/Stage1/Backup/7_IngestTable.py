...
try:
    ...
    #             sys.exit()
                ...
                print(temp)
                print(tbname)
                tables = "(SELECT "+temp+" FROM ["+tbname+"]  WHERE date_ >="+past+") AS data1"
                if tbname1 == 'G_LEntry':
                ...
                prevTab=prevTab.filter(prevTab.date_<past)
                #prevTab.cache()
    ...
    #             sys.exit()
                ...
                print(temp)
                print(tbname)
                tables = "(SELECT "+temp+" FROM ["+tbname+"]  WHERE date_ <="+present+") AS data1"
                if tbname1 == 'G_LEntry':
                    table = sqlctx.read.format("jdbc").options(url=Sqlurl,dbtable=tables,driver="com.microsoft.sqlserver.jdbc.SQLServerDriver",partitionColumn="Entry No_",\
                        lowerBound=0, upperBound=10000000, numPartitions=3).load()
                elif tbname1 == 'ValueEntry':
