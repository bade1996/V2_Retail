...
Start_Year = 2018
try:
    os.system("/home/padmin/hadoop-3.2.2/bin/hadoop fs -rm -r /KOCKPIT/Market/Stage2/Stock_Ageing")
    if(os.system("/home/padmin/hadoop-3.2.2/bin/hdfs dfs -test -d /KOCKPIT/Market/Stage2/Stock_Ageing") !=0):
        ...
        while(Start_Year <= End_Year):
            ...
            try:
                ...
                if(cdm_back == '201803'):
                    ...
                    stock_back_month = sqlctx.read.parquet(hdfspath+"/Market/Stage2/Stock_Ageing/YearMonth="+cdm_back)
                ...
                if(cdm_back == '201803'):
                    ...
                    stock = sqlctx.read.parquet(hdfspath+"/Market/Stage2/Stock_Ageing/YearMonth="+cdm_back)
            ...
            if(stock.count() > 0):
                ...
                stock.coalesce(1).write.mode("append").partitionBy("YearMonth").save(hdfspath+"/Market/Stage2/Stock_Ageing")
                if(cdm == '201804'):
                    write_data_sql(stock,"Stock_AgeingI",owmode)
                else:
                    write_data_sql(stock,"Stock_AgeingI","append")
                print("stock Done")
                end_time = datetime.now()
                endtime = end_time.strftime('%H:%M:%S')
                etime = str(end_time-start_time)
                etime = etime.split('.')[0]
                log_dict = [{'Date':Datelog,'Start_Time':stime,'End_Time':endtime,'Run_Time':etime,'File_Name':'Stock_AgeingI','DB':DB,'EN':Etn,
                    'Status':'Completed','ErrorLineNo':'NA','Operation':'Full','Rows':str(stock.count()),'BeforeETLRows':'NA','AfterETLRows':'NA'}]
