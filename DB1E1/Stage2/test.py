...
conf = SparkConf().setMaster(smaster).setAppName("Margintest")\
...
print(datetime.now())
try:
    if(os.system("/home/padmin/hadoop-3.2.2/bin/hdfs dfs -test -d /KOCKPIT/Market/Stage2/Margintest") !=0):
        ...
        if(sales.count() > 0):
            ...
            sales=sales.join(item,'item_det_code','left').drop('item_det_code')
            ...
            sales= sales.withColumn('MargintestValue',sales['mrp_lot']-sales['Discount_Amount']-sales['CostPrice'])
            sales= sales.withColumn('BUSINESS_VALUE_PER_UNIT_%_TO_SP',((sales['mrp_lot']-sales['CostPrice'])/sales['mrp_lot']))
            sales= sales.withColumn('Net_Margintest_Value',sales['MargintestValue']-sales['GST_Diff'])
            sales= sales.withColumn('Margintest_VALUE_PER_UNIT_%_TO_SP',(sales['Net_Margintest_Value']/sales['mrp_lot']))
            ...
            sales = sales.groupby('vouch_date','Branch_Code','lot_code','vouch_num','YearMonth','Cashier_Code','basic_rate_lot','GST',\
                                  ...
                                  .agg(sum('Tot_Qty').alias('Sales_Quantity'),sum('NET_SALE_VALUE').alias('NET_SALE_VALUE'),sum('Discount_Amount').alias('Discount_Amount'),\
                                        sum('Total_GST_Amount_Paid').alias('Tax1'),sum('Calc_tax_3').alias('Tax3'),sum('Gross_Amount').alias('Gross_Amount'),sum('BASIC_SELLING_PRICE').alias('BASIC_SELLING_PRICE')\
                                         ,sum('GST_INPUT').alias('GST_INPUT'),sum('GST_OUTPUT').alias('GST_OUTPUT'),sum('GST_Diff').alias('GST_Diff'),sum('MargintestValue').alias('MargintestValue'),\
                                         sum('CostPrice').alias('CostPrice'),sum('BUSINESS_VALUE_PER_UNIT_%_TO_SP').alias('BUSINESS_VALUE_PER_UNIT_%_TO_SP'),sum('Net_Margintest_Value').alias('Net_Margintest_Value')\
                                         ,sum('Margintest_VALUE_PER_UNIT_%_TO_SP').alias('Margintest_VALUE_PER_UNIT_%_TO_SP'))
                      
            sales.cache()
            print(sales.count())
            sales.coalesce(1).write.mode("append").partitionBy("YearMonth").save(hdfspath+"/Market/Stage2/Margintest")
            end_time = datetime.now()
            endtime = end_time.strftime('%H:%M:%S')
            etime = str(end_time-start_time)
            etime = etime.split('.')[0]
            log_dict = [{'Date':Datelog,'Start_Time':stime,'End_Time':endtime,'Run_Time':etime,'File_Name':'Margintest','DB':DB,'EN':Etn,
        ...
        for i in range(0,3):
            ...
            try:
                ...
                os.system("/home/padmin/hadoop-3.2.2/bin/hadoop fs -rm -r /KOCKPIT/Market/Stage2/Margintest/YearMonth="+cdm)
                item = sqlctx.read.parquet(hdfspath+"/Market/Stage2/Item")
                item=item.select('GST_CATEGORY','item_det_code')
                item=item.withColumn('GST1', regexp_replace(col("GST_CATEGORY"), "GST ", ""))
                item=item.withColumn('GST', (regexp_replace(col("GST1"), "%", "")/100))
                ...
                sales = sales.groupby('vouch_date','Branch_Code','lot_code','vouch_num','YearMonth','Cashier_Code','basic_rate_lot','GST',\
                                      ...
                                      .agg(sum('CostPrice').alias('CostPrice'),sum('Tot_Qty').alias('Sales_Quantity'),sum('NET_SALE_VALUE').alias('NET_SALE_VALUE'),sum('Discount_Amount').alias('Discount_Amount'),\
                                           sum('Total_GST_Amount_Paid').alias('Tax1'),sum('Calc_tax_3').alias('Tax3'),sum('Gross_Amount').alias('Gross_Amount')),sum('BASIC_SELLING_PRICE').alias('BASIC_SELLING_PRICE')\
                                            ,sum('GST_INPUT').alias('GST_INPUT'),sum('GST_OUTPUT').alias('GST_OUTPUT'),sum('GST_Diff').alias('GST_Diff'),sum('MargintestValue').alias('MargintestValue'),\
                                            sum('BUSINESS_VALUE_PER_UNIT_%_TO_SP').alias('BUSINESS_VALUE_PER_UNIT_%_TO_SP'),sum('Net_Margintest_Value').alias('Net_Margintest_Value')\
                                            ,sum('Margintest_VALUE_PER_UNIT_%_TO_SP').alias('Margintest_VALUE_PER_UNIT_%_TO_SP')
                ...
                print("Margintest Done")
            except Exception as e:
                print(e)
        print(datetime.now())
        TXN = sqlctx.read.parquet(hdfspath+"/Market/Stage2/Margintest")
        #TXN.show()
        TXN = TXN.drop("YearMonth")
        write_data_sql(TXN,"Margintest",owmode)
        end_time = datetime.now()
        endtime = end_time.strftime('%H:%M:%S')
        etime = str(end_time-start_time)
        etime = etime.split('.')[0]
