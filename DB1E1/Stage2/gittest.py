...
conf = SparkConf().setMaster(smaster).setAppName("GITTEST")\
...
print(datetime.now())
os.system("/home/padmin/hadoop-3.2.2/bin/hadoop fs -rm -r /KOCKPIT/Market/Stage2/GITTEST")
...
sales = sqlctx.read.parquet(hdfspath+"/Market/Stage1/Sales").where(col('YearMonth').isin([202201,202202,202203,202204]))
...
print(sales.count())
sales.cache()
Purchase = sqlctx.read.parquet(hdfspath+"/Market/Stage1/Purchase").where(col('YearMonth').isin([202201,202202,202203,202204]))
...
Date_format_range = Date_format_range.filter(Date_format_range['Key'] == 202204)
...
sales = sales.drop('Branch_Code','item_det_code','vouch_num','vouch_date','Num_Item')     
...
sales = sales.withColumn('GITTEST', sumf('Tot_Qty').over(my_window).alias('GITTEST'))
...
sales = sales.filter(sales['GITTEST'] != 0)
sales.coalesce(1).write.mode("append").partitionBy("YearMonth").save(hdfspath+"/Market/Stage2/GITTEST")
write_data_sql(sales,"GITTEST",owmode)
print(datetime.now())