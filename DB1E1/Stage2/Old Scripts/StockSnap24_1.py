'''
Creators: Mr Abhishek, Aniket
DEPENDENT ON Stage1 7.2_Ing_Stkdtxn
LAST TWO MONTHS ROLLING 
To change Sink/Dump Destination, change the table name at write positions, also in the script where DELETE Query is present(2months rolling part)
'''
...
try:
    ...
    conf = SparkConf().setMaster(smaster).setAppName("StockSnapshot24Mth")\
...
#         .set("spark.network.timeout", "6000s")
    ...
    .config("spark.executor.heartbeatInterval", "1000000").getOrCreate()
...
#     lot.show()
#     exit()
    stock = sqlctx.read.parquet(hdfspath+"/"+DBET+"/Stage1/stk_dtxn2")
    '''
    ...
    '''
    stock=stock.select("Net_Qty","Txn_Type","Lot_Code","Godown_Code","Branch_Code","Date_","yearmonth","sl_qty","sr_qty")
    
    stock = stock.filter(stock.Godown_Code ==0)
    #stock.write.mode("overwrite").save(hdfspath+"/"+DBET+"/Stage2/StockAllYear")
    ...
    Calendar_Startdate = Calendar_Enddate - relativedelta.relativedelta(months=24)     ##### 24 mnths snapshot 31 March
    ...
    try:
        ############ROW COUNT##############
        cntQuery = "(SELECT COUNT(*) As kalam FROM StockSnapshot2) As Data"
        ...
        if table.count()>=24:
            ...
            cond= [(stock["Date_"]<=records.Link_date) &(stock["EndDate1"]>=records.Link_date)]   #Later added '=' in the later part
            #print("joint")
            ageing = stock.join(records,cond,"inner")
            
            
            lot=sqlctx.read.parquet(hdfspath+"/"+DBET+"/Stage2/Lot")         
            lot=lot.select("Lot_Code","pur_rate","sale_rate","Item_Det_Code","pur_date","lot_number","lot_basic_rate","lot_mrp",\
               "sp_rate1","sp_rate2","Expiry","Mfg_Date")
            ...
            etime = etime.split('.')[0]
            log_dict = [{'Date':Datelog,'Start_Time':stime,'End_Time':endtime,'Run_Time':etime,'File_Name':'StockSnap24','DB':DB,'EN':Etn,
                'Status':'Completed','ErrorLineNo':'NA','Operation':'Incremental','Rows':str(ageing.count()),'BeforeETLRows':str(bcnt),'AfterETLRows':str(cnt)}]
            ...
            print("ELSE FULL RELOAD 24 mths")
            ...
            etime = etime.split('.')[0]
            log_dict = [{'Date':Datelog,'Start_Time':stime,'End_Time':endtime,'Run_Time':etime,'File_Name':'StockSnap24','DB':DB,'EN':Etn,
                'Status':'Completed','ErrorLineNo':'NA','Operation':'Full','Rows':str(ageing.count()),'BeforeETLRows':'0','AfterETLRows':str(cnt)}]
            log_df = sqlctx.createDataFrame(log_dict,schema_log)
            log_df.write.jdbc(url=Sqlurlwrite, table="Logs", mode="append")
