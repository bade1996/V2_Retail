...
try:
    ...
    sch=sch.filter(sch.code_type=='SCHCMP')
#     sch.distinct("code_type").show()
    sch.filter(sch.sl_txn_code==26).show()
    exit()
...
#     sales = sales.filter(sales.vouch_date=='2020-03-14')
    ...
    sales=sales.groupBy("Type","Item_Det_Code","Lot_Code","Voucher_Date","FiscalYear","Branch_Code","cust_code","series_code","Code_Drop")\
    ...
    print("Sales Writing")
    sales.write.jdbc(url=Sqlurlwrite, table="salesTEST", mode="overwrite")#, driver="com.microsoft.sqlserver.jdbc.SQLServerDriver")
